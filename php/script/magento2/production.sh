...
echo `/sbin/ip route|awk '/default/ { printf("%s\t", $3) }'` $MAGENTO_HOST_URL >> /etc/hosts
...
if [ "$USE_SAMPLE_DATA" = true ]; then
  mkdir -p $MAGENTO_FOLDER/var/composer_home/ \
    && cp /root/.composer/auth.json $MAGENTO_FOLDER/var/composer_home/auth.json
...
chown -R www-data:www-data $MAGENTO_FOLDER
